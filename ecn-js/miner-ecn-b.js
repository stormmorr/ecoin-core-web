...
function ag_CompileTarget(response)
{
	...
	for(var f_ElindexID = 0; f_ElindexID < hollynom; f_ElindexID++)
		{
		f_Collection.m_vec_Element[tcob[f_ElindexID][0]].m_vec_Element[f_Collection.m_vec_Element[tcob[f_ElindexID][0]].m_idx_vec_Element++] = f_Collection.m_vec_Element[tcob[f_ElindexID][1]];
		f_Collection.m_vec_Element[tcob[f_ElindexID][0]].m_vec_Leg[f_Collection.m_vec_Element[tcob[f_ElindexID][0]].m_idx_vec_Leg++] = f_Collection.m_vec_Element[tcob[f_ElindexID][1]].m_vec_Leg[0];
		}
	...
	console.log("Loaded classTarget ID 1");
...
function ag_Gen_Hesha(job, progress_report, cb)
{
	...
	job.targdiff = 105000.5; //Target Mark Threshold
	var f_TargetFails = 0;
	job.targdiff += 1.0;
	job.save = false;
	job.end = false;
    ...
    while(run)
		...
		var f_LimitSense = (CUBE_ADMAX * 0.9) * Math.random();
		...
		var g_Result = g_Target.acCompare(f_Target, 18.55, job.targdiff, true);
