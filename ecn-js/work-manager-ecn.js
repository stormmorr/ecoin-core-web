...
function onSuccess(jsonresp)
{
	...
	if(0)
        ...
        worker = { postMessage : function(m) { worker.intMessage( { data: m} ); },
                   intMessage: function() {} };
        var m = readScript('ecn-js/miner-ecn.js');
        ...
        worker = new Worker("ecn-js/miner-ecn.js");
...
function eSession()
{
	...
	/*ag_GatherTransactions(f_JobID, f_Block, function(f_Results)
		{
		console.log("Gathered TX: " + f_Results);
	...
	if(f_Save == true)
		...
		$.post("link-request-getwork-ecn-fast.php", {type: "GWQ_SELECT", query: "SELECT id, jobid, blockledger, dated FROM block ORDER BY dated DESC LIMIT 1"}, function(data, status)
			...
			ag_GatherTransactions(g_JobID, f_Block, function(f_Results, f_Block, f_rotoner, f_TxLCount)
				...
				if((f_Block == f_TxLCount) && (f_rotoner <= 0) && (f_EBlocker == 1))
					{
					f_EBlocker = 0;
